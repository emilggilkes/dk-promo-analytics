...
def compute_cate():
    ...
    for _, camp in campaigns.iterrows():
        ...
        # CI from per-tree prediction variance within the forest
        # Each tree gives a slightly different CATE — spread = uncertainty
        tree_preds = np.array([
            tree.predict(X) for tree in tau_model.estimators_
        ])  # shape: (n_trees, n_users)
